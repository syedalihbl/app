...
class ClientService {
  ...
  async perform(apiRequest) {
    ...
    apiRequest.body.BASICNO = apiRequest.pathParams.AccountId
    ...
    const result = await this.client.performRestRequest(apiRequest.headers, apiRequest.body, config.get("api.getTransactionByCIF.v1.method"), config.get("api.getTransactionByCIF.v1.url"));
