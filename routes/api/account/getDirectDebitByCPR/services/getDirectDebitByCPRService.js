...
const Req_GetDirectDebitByCPRModel = require('../model/Req_GetDirectDebitByCPRModel');
const config = require('config');

class getDirectDebitByCPR {
    ...
    constructor(headers, pathParams, queryParams, logger) {
        ...
        this.getDirectDebitByCPRModel = new Req_GetDirectDebitByCPRModel({ ReqType: queryParams.ReqType, Channel: queryParams.Channel, ReqUID: queryParams.ReqUID, cprNo: queryParams.cprNo })
    ...
    async perform() {
        try {
            ...
            if (headerValidationResponse.error) {
                ...
                const paramValidationResponse = this.getDirectDebitByCPRModel.validateSchema();
                ...
                if (paramValidationResponse.error) {
                    ...
                    const apiRequest = new APIRequest(this.commonHeader, {}, {}, this.getDirectDebitByCPRModel)
                    // console.log(apiRequest)
                    this.logger.debug({ apiRequest: apiRequest }, 'standing order API Request')
    
                    const clientService = new ClientService(config.get('api.directDebitByCPR.v1.api_type'), this.logger);
                    ...
                    if (response.error) {
                        if (response.error.code == 'ECONNREFUSED') {
                            ...
                            return new APIResponse("Fail", {}, new APIError(this.commonHeader.xReqId, response.error.code, response.error.message, 'TargetSystemValidationError', 
                                response.error.developer_message && response.error.developer_message.RESPONSE 
...
module.exports = getDirectDebitByCPR;