...
const Req_FutureDatedPaymentModel = require('../model/Req_FutureDatedPaymentModel');
const config = require('config');

class getFutureDatedPayment {
    ...
    constructor(headers, pathParams, queryParams, logger) {
        ...
        this.getFutureDatedPaymentsModel = new Req_FutureDatedPaymentModel({ ReqType: queryParams.ReqType, Channel: queryParams.Channel, ReqUID: queryParams.ReqUID, accountNo: queryParams.accountNo })
    ...
    async perform() {
        try {
            ...
            const apiRequest = new APIRequest(this.commonHeader, {}, {}, this.getFutureDatedPaymentsModel);

            this.logger.debug(this.commonHeader.xReqId, 'AccountBalance API Request : ', JSON.stringify(apiRequest));
            const paramValidationResponse = this.getFutureDatedPaymentsModel.validateSchema();
            ...
            const clientService = new ClientService(config.get('api.getFutureDatedPayments.api_type'), this.logger);
...
module.exports = getFutureDatedPayment;