...
const getFutureDatedPaymentsService = require('../services/getFutureDatedPaymentService');

const getFutureDatedPayments = function (req, res) {
    const logger = loggerJs.pinoChildInstance(req.headers['x-channel-id'], { reqId: req.headers['x-req-id'] })
    let service = new getFutureDatedPaymentsService(req.headers, req.params, req.query, logger);
    ...
    service.perform().then((result) => {
        ...
        logger.info({ statusCode: 500, responseController: error.stack }, 'In Conroller IVR Block Inquiry Service V1');
        return res;
    });
    return res;
};
module.exports = {
    getFutureDatedPayments: getFutureDatedPayments
}; 