...
const Req_GetCPRByCIFModel = require('../model/Req_GetCPRByCIFModel');
...
class getCPRByCIF {
    ...
    constructor(headers, pathParams, queryParams, logger) {
        ...
        this.getCPRByCIFModel = new Req_GetCPRByCIFModel({ ReqType: queryParams.ReqType, Channel: queryParams.Channel, ReqUID: queryParams.ReqUID, cifNo: queryParams.cifNo })
    ...
    async perform() {
        try {
            ...
            if (headerValidationResponse.error) {
                ...
                const paramValidationResponse = this.getCPRByCIFModel.validateSchema();
                ...
                if (paramValidationResponse.error) {
                    ...
                    const apiRequest = new APIRequest(this.commonHeader, {}, {}, this.getCPRByCIFModel)
                    // console.log(apiRequest)
                    this.logger.debug({ apiRequest: apiRequest }, 'standing order API Request')
    
                    const clientService = new ClientService(config.get('api.getCPRByCIF.v1.api_type'), this.logger);
