...
const getCPRByCIFService = require('../services/getCPRByCIFService');

const getCPRByCIF = function (req, res) {
    // console.log('in germ')
    const logger = loggerJs.pinoChildInstance(req.headers['x-channel-id'], { reqId: req.headers['x-req-id'] })
    let service = new getCPRByCIFService(req.headers, req.params, req.query, logger);
