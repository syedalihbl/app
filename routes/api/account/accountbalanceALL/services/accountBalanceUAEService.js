...
class accountBalanceService {
    ...
    async perform() {
        try {
            ...
            const clientService = new ClientService(config.get('api.accountBalanceBAHv2.v2.api_type'), this.logger);

            this.logger.debug('ClientService object Constructed : ', clientService);
            performance.mark('calling-upstream')
            const response = await clientService.perform(apiRequest);
            performance.mark('upstream-call-start')
            ...
            if (response.error) {
                if (response.error.code == 'ECONNREFUSED') {
                    ...
                    return new APIResponse("Fail", {}, new APIError(this.commonHeader.xReqId, response.error.code, response.error.message, 'TargetSystemValidationError', JSON.stringify(response.error.developer_message.message)));
                }
            }
            if (apiRequest.headers.xCountryCode == 'BAH') {
                if (response.resCode == '0000000') {
                    delete response.resCode;
                    delete response.resDesc;
                    delete response.developer_message;

                    return new APIResponse("Success", response, {});
                } else if (response.resCode !== '0000000') {
                    return new APIResponse("Fail", {}, new APIError(this.commonHeader.xReqId, response.resCode, response.resDesc, 'TargetSystemValidationError', response.developer_message.RESPMESSAGE));
                }
                else {
            ...
            else if (apiRequest.headers.xCountryCode == 'UAE') {
                if (response.developer_message.ACCOUNTTYPE == '') {
