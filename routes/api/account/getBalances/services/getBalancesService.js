...
const Req_GetBalancesModel = require('../model/Req_GetBalancesModel');
...
class getBalances {
    ...
    constructor(headers, pathParams, queryParams, logger) {
        ...
        this.GetBalancesModel = new Req_GetBalancesModel({ ReqType: queryParams.ReqType, Channel: queryParams.Channel, ReqUID: queryParams.ReqUID, accountNo: pathParams.accountNo })
    ...
    async perform() {
        try {
            ...
            this.logger.debug(this.commonHeader.xReqId, 'Bahrain Get Balance API Request : ', JSON.stringify(apiRequest));
            ...
            const clientService = new ClientService(config.get('api.getBalances.v2.api_type'), this.logger);

            this.logger.debug('ClientService object Constructed : ', clientService);
            const response = await clientService.perform(apiRequest);
            //console.log(response, 'response.developer_message')
