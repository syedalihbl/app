...
const Req_AccountBalanceModel = require('../model/Req_AccountBalanceModel');
...
class accountBalanceService {
    ...
    async perform() {
        try {
            ...
            if (response.error) {
             return new APIResponse("Fail", {}, new APIError(this.commonHeader.xReqId, '', 'Something went wrong to fullfull the results, Kindly contact with administrator.', 'ValidationError', response.error));
            }
            if (JSON.stringify(response) == '{}') {
                return new APIResponse("Fail", response, new APIError(this.commonHeader.xReqId, '', 'Something went wrong to fullfull the results, Kindly contact with administrator.', 'ValidationError', response));
            }
