...
class getTransactions {
    ...
    constructor(headers, pathParams, queryParams, logger) {
        ...
        this.GetTransactionModel = new Req_GetTransactionModel({ ReqType: queryParams.ReqType, Channel: queryParams.Channel, ReqUID: queryParams.ReqUID, accountNo: pathParams.AccountId })
        this.logger.debug('Common Headers Object Constructed : ', this.commonHeader);
        console.log(this.GetTransactionModel)
    }
        ...
        try {
            ...
            if (headerValidationResponse.error) {
                ...
                if (paramValidationResponse.error) {
                    ...
                    const clientService = new ClientService(config.get('api.getTransactionByCPR.v1.api_type'), this.logger);
                    ...
                    if (response.error) {
                        if (response.error.code == 'ECONNREFUSED') {
                            ...
                            return new APIResponse("Fail", {}, new APIError(this.commonHeader.xReqId, response.error.code, response.error.message, 'TargetSystemValidationError', JSON.stringify(response.error.developer_message.RESPONSE)));
                        }
                    }
