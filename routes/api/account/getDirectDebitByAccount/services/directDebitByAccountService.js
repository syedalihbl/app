...
class directDebitByAccountService {
    ...
    constructor(headers, pathParams, queryParams, logger) {
        ...
        this.directDebitByAccountModel = new Req_DirectDebitByAccountModel({ ReqType: queryParams.ReqType, Channel: queryParams.Channel, ReqUID: queryParams.ReqUID, pathParams })
    ...
    async perform() {
        try {
            ...
            if (headerValidationResponse.error) {
                ...
                const paramValidationResponse = this.directDebitByAccountModel.validateSchema();
                ...
                if (paramValidationResponse.error) {
                    ...
                    const apiRequest = new APIRequest(this.commonHeader, this.body, {}, this.directDebitByAccountModel);
                    // console.log(apiRequest)
                    this.logger.debug({ apiRequest: apiRequest }, 'DirectDebitbyAccount API Request')
    
                    const clientService = new ClientService(config.get('api.directDebitByAccount.v1.api_type'), this.logger);
                    ...
                    if (response.responseCode == '0000') {
                        ...
                        return new APIResponse("Success", response.outparams, {});
            ...
            console.log(error);
            this.logger.error({ errorStack: error.stack }, 'Error while executing DirectDebitbyAccount Validation Service perform method');
...
module.exports = directDebitByAccountService;