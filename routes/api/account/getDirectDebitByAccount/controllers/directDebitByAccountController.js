...
const directDebitByAccountService = require('../services/directDebitByAccountService');
// const Authenticator = require('../../../../../common/authentication/channelsAuth')

const directDebitByAccount = function (req, res) {
    const logger = loggerJs.pinoChildInstance(req.headers['x-channel-id'], { reqId: req.headers['x-req-id'] })
    let service = new directDebitByAccountService(req.headers, req.params, req.query, logger, req.body);
    service.perform().then((result) => {
        logger.debug('Response Recieved from Direct Debit by Account Service : ', result);
        ...
        logger.error({ errorStack: error.stack }, 'Error in Direct Debit by Account V1 Controller while serving client request')
