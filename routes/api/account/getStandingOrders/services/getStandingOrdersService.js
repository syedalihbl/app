...
const Req_GetStandingOrdersModel = require('../model/Req_GetStandingOrdersModel');
const config = require('config');

class getStandingOders {
    ...
    constructor(headers, pathParams, queryParams, logger) {
        ...
        this.getStandingOdersModel = new Req_GetStandingOrdersModel({ ReqType: queryParams.ReqType, Channel: queryParams.Channel, ReqUID: queryParams.ReqUID, cprNo: queryParams.cprNo })
    ...
    async perform() {
        try {
            ...
            if (headerValidationResponse.error) {
                ...
                const paramValidationResponse = this.getStandingOdersModel.validateSchema();
                ...
                if (paramValidationResponse.error) {
                    ...
                    const apiRequest = new APIRequest(this.commonHeader, {}, {}, this.getStandingOdersModel)
                    // console.log(apiRequest)
                    this.logger.debug({ apiRequest: apiRequest }, 'standing order API Request')
    
                    const clientService = new ClientService(config.get('api.getStandingOrders.api_type'), this.logger);
                    ...
                    if (response.error) {
                        if (response.error.code == 'ECONNREFUSED') {
                            ...
                            return new APIResponse("Fail", {}, new APIError(this.commonHeader.xReqId, response.error.code, response.error.message, 'TargetSystemValidationError', response.error.developer_message && response.error.developer_message.RESPONSE 
                                ? response.error.developer_message.RESPONSE.REPMESSAGE 
                                : 'Unknown error'));
                        }
                    }
...
module.exports = getStandingOders;