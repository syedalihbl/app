...
const getStandingOrdersService = require('../services/getStandingOrdersService');

const getStandingOrders = function (req, res) {
    // console.log('in germ')
    const logger = loggerJs.pinoChildInstance(req.headers['x-channel-id'], { reqId: req.headers['x-req-id'] })
    let service = new getStandingOrdersService(req.headers, req.params, req.query, logger);
    ...
    service.perform().then((result) => {
        ...
        logger.error({ errorStack: error.stack }, 'Error in getStandingOrder V1 Controller while serving client request')
        //logger.error('Error in Account Detail Inquiry Controller while serving client request : ', error.stack);
        res.status(500).json(error.stack);
        logger.info({ statusCode: 500, responseController: error.stack }, 'In Conroller getStandingOrder Service V1');
        return res;
    });
    return res;
};
