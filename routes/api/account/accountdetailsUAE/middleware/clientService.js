...
class ClientService {
    ...
    async perform(apiRequest) {        
        ...
        const result = await this.client.performRestRequest(apiRequest.headers, apiRequest.pathParams.accountNo,  config.get('api.accountDetailsUAE.v1.method'), config.get('api.accountDetailsUAE.v1.url')+"?AccountNumber="+apiRequest.pathParams.accountNo+'&ApprovedListValidation=N');
