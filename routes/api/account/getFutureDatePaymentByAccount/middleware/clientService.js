...
class ClientService {
  constructor(client_name, logger) {
    this.logger = logger;
    this.logger.debug("Future Date Service client_name : ", JSON.stringify(client_name));
  ...
  async perform(apiRequest) {
    ...
    const result = await this.client.performRestRequest({}, payload, config.get("api.getFutureDate.v2.method"), config.get("api.getFutureDate.v2.url"));
