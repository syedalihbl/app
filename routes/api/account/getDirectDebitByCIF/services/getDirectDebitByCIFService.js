...
const Req_GetBalancesModel = require('../model/Req_GetDirectDebitByCIFModel');
const config = require('config');

class getDirectDebitByCIFService {
    ...
    constructor(headers, pathParams, queryParams, logger) {
        ...
        this.GetBalancesModel = new Req_GetBalancesModel({ ReqType: queryParams.ReqType, Channel: queryParams.Channel, ReqUID: queryParams.ReqUID, cifNo: queryParams.cifNo })
    ...
    async perform() {
        try {
            ...
            const apiRequest = new APIRequest(this.commonHeader, {}, {}, this.GetBalancesModel);
            this.logger.debug(this.commonHeader.xReqId, 'Bahrain GET Debit By CIF No API Request : ', JSON.stringify(apiRequest));
            const paramValidationResponse = this.GetBalancesModel.validateSchema();
            this.logger.debug('Request Body Validation Performed : ', paramValidationResponse);

            if (paramValidationResponse.error) {
                this.logger.debug('Request Parameter Debit by CIF No Fails, Raised API Error with following Parameters : ', this.commonHeader.xReqId, '', paramValidationResponse.error.details[0].message, paramValidationResponse.error.name, paramValidationResponse.error.details);
            ...
            const clientService = new ClientService(config.get('api.directDebitByCIF.v1.api_type'), this.logger);
            ...
            if (response.developer_message.RESPONSE.RESPCODE != '0000') {
            ...
            } else if (response.developer_message.RESPONSE.RESPCODE == '0000') {
...
module.exports = getDirectDebitByCIFService;