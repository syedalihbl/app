...
class ClientService {
    ...
    constructor(clientName, logger) {
        ...
        this.logger.debug({ clientName: clientName }, 'fetchConsentAISP clientName');
    ...
    async perform(apiRequest) {
        ...
        if (apiRequest.headers.xCountryCode.toUpperCase() === 'BAH') {
            ...
            const requestURL = config.get('api.fetchConsentAISP.v1.url')
            const requestMethod = config.get('api.fetchConsentAISP.v1.method')
