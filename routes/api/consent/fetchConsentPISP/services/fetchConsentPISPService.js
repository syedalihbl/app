...
const Req_fetchConsentPISPModel = require('../model/Req_fetchConsentPISPModel')

class fetchConsentPISPService {

    constructor(headers, body, logger) {
        this.logger = logger
        //console.log("Query:", body)

        this.logger.info({ headers: headers, params: body }, 'fetchConsentPISP Service Object Initiated with Headers and Params')
        ...
        this.fetchConsentPISPModel = new Req_fetchConsentPISPModel(body.consentId)
        this.logger.debug({ dataModel: this.fetchConsentPISPModel }, 'Request Model Object Constructed')
    ...
    async perform() {
        try {
            ...
            if (headerValidationResponse.error) {
                ...
                const paramValidationResponse = this.fetchConsentPISPModel.validateSchema();
                ...
                if (paramValidationResponse.error) {
                    ...
                    const apiRequest = new APIRequest(this.commonHeader, this.fetchConsentPISPModel, {}, {})
                    this.logger.debug({ apiRequest: apiRequest }, 'fetchConsentPISP API Request')

                    const clientService = new ClientService(config.get('api.fetchConsentPISP.v1.api_type'), this.logger)
...
module.exports = fetchConsentPISPService;